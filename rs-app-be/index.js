...
async function initDb() {
  try {
    await pool.query('SELECT 1'); // Tes koneksi
    ...
    console.log("Tabel 'users', 'pasien', dan 'tagihan' berhasil disiapkan.");
  } catch (err) {
...
app.post('/pasien', authenticateToken, async (req, res) => {
  try {
    ...
    if (!nama || !umur) {
      return res.status(400).send('Nama dan umur tidak boleh kosong');
    }
    const { rows } = await pool.query(
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`Server backend berjalan di 0.0.0.0:${PORT}`);
  initDb();
});
